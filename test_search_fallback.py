    ...
    for r in results:
        if r.shard == 0 and r.sample_index == 65357:
            ...
            if r.token_offset == 1671:
                ...
                print(f"  Specific failure test failed. Expected offset 1671, got {r.token_offset}")
            break
...
def test_randomized(n_tests=10):
    ...
    for i in range(n_tests):
        ...
        print(f"Test {i+1}/{n_tests}: Target: {decoded!r} (Shard {shard}, Sample {sample}, Offset {start})")
        ...
        for r in results:
            if r.shard == shard and r.sample_index == sample:
                if r.token_offset == start or r.token_offset == start - 1:
                    ...
                    print(f"  Offset mismatch! Reported: {r.token_offset}, Actual: {start}")
                    found = True # Consider it found for reporting
                break
                
        if not found:
             print(f"  Document not found in Infini-Gram results (might be expected if sequence is too long or not indexed).")
